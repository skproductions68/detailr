  ...
  <div
    ...
    className={`p-6 rounded-none border transition-all duration-300 ${
      selected
        ...
        : "bg-black text-gray-300 border-white/10 hover:border-white/30 hover:bg-white/5"
    } ${className}`}
  >
    {children}
  </div>
    ...
    <div className="max-w-4xl mx-auto pt-20 px-6 pb-24">
      ...
      <div className="flex justify-between items-end mb-12">
        ...
        <div className="flex gap-1.5">
          {[1, 2, 3, 4].map((s) => (
            <div key={s} className={`h-0.5 w-10 transition-all ${s <= step ? "bg-white" : "bg-white/10"}`} />
          ))}
        </div>
      </div>
